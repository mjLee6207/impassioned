...
<div class="main-wrap">
    ...
    <div class="board-wrap">
        ...
        <div style="margin-bottom: 18px;">
            ...
            작성자: <b>${board.nickname}</b> | 작성일: ${board.writeDate} | 조회수: ${board.viewCount}
        </div>
                ...
                <c:otherwise>
                    <div class="comment-write-wrap">
                        <form action="/board/review/add.do" method="post" class="comment-form">
                            <input type="hidden" name="boardId" value="${board.boardId}">
                            <textarea name="content" class="comment-textarea" id="commentContent" maxlength="300" required
...
<script>
    ...
    $(document).ready(function () {
        ...
        $.get("/countLike.do", { boardId }, function (count) {
            $("#likeCountText").text(`좋아요 ${count}개`);
        });
        ...
        $btn.on("click", function () {
            ...
            $.ajax({
                ...
                success: function () {
                    ...
                    $.get("/countLike.do", { boardId }, function (res) {
                        const count = (typeof res === "object" && res.count !== undefined) ? res.count : res;
                        const finalCount = isNaN(count) ? 0 : parseInt(count);
                        $("#likeCountText").text(`좋아요 ${finalCount}개`);
                    });
                } // ← 이 중괄호만 위치 수정한 것
            });
        });
